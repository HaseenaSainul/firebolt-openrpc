...
cmake -Hsrc/Thunder -Bbuild/Thunder \
      -DCMAKE_INSTALL_PREFIX=${THUNDER_INSTALL_DIR}/usr \
      -DCMAKE_MODULE_PATH=${THUNDER_INSTALL_DIR}/tools/cmake \
      -DCMAKE_BUILD_TYPE=Debug -DBINDING=127.0.0.1 -DPORT=55555 -DENABLE_CODE_COVERAGE=no -DHIDE_NON_EXTERNAL_SYMBOLS=OFF -DMESSAGING=ON -DTRACING=OFF -DBUILD_LIB_AS_STATIC=OFF
#-DPROCESS=OFF -DPLUGINS=OFF -DEXECUTABLE=OFF
fi
